...
from src.domain.repositories.camera_repository import ICameraServiceRepository
...
class CleanICameraRepository(ICameraRepository):
    """Clean architecture implementation of camera repository."""
    
    def __init__(self, camera_service: ICameraServiceRepository):
    ...
    async def get_camera(self, camera_id: str) -> Optional[Camera]:
        ...
        try:
            ...
            return Camera(
                ...
                last_seen=utc_now() if status["is_active"] else None
            )
        ...
        try:
            ...
            for camera_info in camera_list:
                camera = Camera(
                    ...
                    created_at=utc_now(),  # We don't have the actual creation time
                    last_seen=utc_now() if camera_info["is_active"] else None
                )
    ...
    async def update_camera_status(self, camera_id: str, is_active: bool) -> bool:
        ...
        try:
            ...
            if is_active and not camera.is_active:
                ...
                success, _ = self._camera_service.stop_camera(camera_id)
                return success
    ...
    async def stop_all_cameras(self) -> bool:
        """Stop all cameras."""
        try:
            stopped_count = self._camera_service.stop_all_cameras()
